...
>>>>>>> ddf4b57 (feat: add devcontainer)
Object.defineProperty(exports, "__esModule", { value: true });
exports.RPCDeploymentStrategy =
  exports.ContractDeploymentError =
  exports.GasEstimationError =
  exports.TransactionFailedError =
  exports.RPCConnectionError =
    void 0;
    ...
    if (this.verbose) {
      console.log(
        chalk_1.default.blue(
          `🔗 RPC Strategy initialized with endpoint: ${rpcUrl}`,
        ),
      );
      console.log(
        chalk_1.default.blue(
          `👤 Deployer address: ${this.signer.getAddress()}`,
        ),
      );
    }
  }
...
=======
  ...
  async estimateGasWithMultiplier(contract, methodName, args = []) {
    try {
      const estimatedGas = await contract[methodName].estimateGas(...args);
      const gasWithMultiplier = BigInt(
        Math.floor(Number(estimatedGas) * this.gasLimitMultiplier),
      );
      if (this.verbose) {
        console.log(
          chalk_1.default.gray(
            `⛽ Gas estimate for ${methodName}: ${estimatedGas.toString()} (with multiplier: ${gasWithMultiplier.toString()})`,
          ),
        );
      }
      return gasWithMultiplier;
      ...
      throw new GasEstimationError(
        `Failed to estimate gas for ${methodName}: ${error.message}`,
        error,
      );
    }
  }
...
>>>>>>> ddf4b57 (feat: add devcontainer)
        if (this.verbose) {
          console.log(
            chalk_1.default.gray(
              `⛽ Gas price: ${ethers_1.ethers.formatUnits(feeData.gasPrice, "gwei")} gwei`,
            ),
          );
        }
<<<<<<< HEAD
...
=======
        ...
        if (this.verbose) {
          console.log(
            chalk_1.default.gray(
              `⛽ Gas price (fallback): ${ethers_1.ethers.formatUnits(gasPriceBigInt, "gwei")} gwei`,
            ),
          );
>>>>>>> ddf4b57 (feat: add devcontainer)
        }
  ...
  async deployContract(contractName, constructorArgs = [], diamond) {
    return await this.withRetry(async () => {
      try {
        ...
        const artifactPath = (0, path_1.join)(
        ...
        // Create contract factory
        const factory = new ethers_1.ContractFactory(
          artifact.abi,
          artifact.bytecode,
          this.signer,
        );
        ...
        const gasLimit = await factory
          ...
          .then((tx) =>
            BigInt(
              Math.floor(Number(tx.gasLimit || 0) * this.gasLimitMultiplier),
            ),
          );
        // Get gas price
        const gasPrice = await this.getGasPrice();
        if (this.verbose) {
          console.log(
            chalk_1.default.blue(
              `🚀 Deploying ${contractName} with gas limit: ${gasLimit.toString()}`,
            ),
          );
        }
        // Deploy contract
...
>>>>>>> ddf4b57 (feat: add devcontainer)
        ...
        if (this.verbose) {
          ...
          console.log(
            chalk_1.default.gray(
              `   Transaction hash: ${deploymentReceipt.hash}`,
            ),
          );
          console.log(
            chalk_1.default.gray(
              `   Gas used: ${deploymentReceipt.gasUsed.toString()}`,
            ),
          );
        }
        return contract;
        ...
        throw new ContractDeploymentError(
          `Failed to deploy ${contractName}: ${error.message}`,
          contractName,
          error,
        );
      }
    ...
    if (this.verbose) {
      console.log(
        chalk_1.default.blueBright(
          `🚀 Explicitly deploying DiamondCutFacet and Diamond for ${diamond.diamondName} via RPC`,
        ),
      );
    }
<<<<<<< HEAD
...
=======
    try {
      ...
      const diamondCutFacet = await this.deployContract(
        diamondCutContractName,
        [],
        diamond,
      );
      // Deploy the Diamond contract
      const diamondContractName = await (0, utils_1.getDiamondContractName)(
        diamond.diamondName,
        diamond,
      );
      ...
      const diamondContractAddress = await diamondContract.getAddress();
      // Register the DiamondCutFacet function selectors
      const diamondCutFacetSelectorsRegistry =
        diamondCutFacetFunctionSelectors.reduce((acc, selector) => {
          acc[selector] = {
            facetName: "DiamondCutFacet",
            priority:
              diamond.getFacetsConfig()?.DiamondCutFacet?.priority || 1000,
            address: diamondCutFacetAddress,
            action: types_1.RegistryFacetCutAction.Deployed,
          };
          return acc;
      ...
      console.log(
        chalk_1.default.green(
          `✅ Diamond deployed at ${diamondContractAddress}, DiamondCutFacet at ${diamondCutFacetAddress}`,
        ),
      );
    } catch (error) {
      console.error(
        chalk_1.default.red(
          `❌ Failed to deploy diamond via RPC: ${error.message}`,
        ),
      );
      throw error;
    }
  ...
  async deployFacetsTasks(diamond) {
    ...
    for (const facetName of sortedFacetNames) {
      ...
      const availableVersions = Object.keys(facetConfig.versions || {}).map(
        Number,
      );
      const upgradeVersion = Math.max(...availableVersions);
      if (upgradeVersion > deployedVersion || deployedVersion === -1) {
        if (this.verbose) {
          console.log(
            chalk_1.default.blueBright(
              `🚀 Deploying facet: ${facetName} to version ${upgradeVersion} via RPC`,
            ),
          );
        }
        try {
          // Deploy the facet contract using RPC
          const facetContractName = await (0, utils_1.getContractName)(
            facetName,
            diamond,
          );
          const facetContract = await this.deployContract(
            facetContractName,
            [],
            diamond,
          );
          ...
          // Initializer function Registry
          const deployInit =
            facetConfig.versions?.[upgradeVersion]?.deployInit || "";
          const upgradeInit =
            facetConfig.versions?.[upgradeVersion]?.upgradeInit || "";
          const initFn = diamond.newDeployment ? deployInit : upgradeInit;
          if (initFn && facetName !== deployConfig.protocolInitFacet) {
            diamond.initializerRegistry.set(facetName, initFn);
          }
          const newFacetData = {
            ...
            funcSelectors: facetSelectors,
            deployInclude:
              facetConfig.versions?.[upgradeVersion]?.deployInclude || [],
            deployExclude:
              facetConfig.versions?.[upgradeVersion]?.deployExclude || [],
            initFunction: initFn,
            verified: false,
          };
          diamond.updateNewDeployedFacets(facetName, newFacetData);
          console.log(
            chalk_1.default.cyan(
              `⛵ Deployed at ${await facetContract.getAddress()} with ${facetSelectors.length} selectors.`,
            ),
          );
          if (this.verbose) {
            console.log(chalk_1.default.gray(`  Selectors:`), facetSelectors);
          ...
          console.error(
            chalk_1.default.red(
              `❌ Failed to deploy facet ${facetName}: ${error.message}`,
            ),
          );
          throw error;
        }
  ...
  async performDiamondCutTasks(diamond) {
    ...
    try {
      ...
      const diamondCutArtifactPath = (0, path_1.join)(
        diamond.contractsPath,
        "interfaces",
        "IDiamondCut.sol",
        "IDiamondCut.json",
      ...
      if (!fs.existsSync(diamondCutArtifactPath)) {
        throw new Error(
          `IDiamondCut artifact not found at: ${diamondCutArtifactPath}`,
        );
      }
      const diamondCutArtifact = JSON.parse(
        fs.readFileSync(diamondCutArtifactPath, "utf8"),
      );
      const diamondContract = new ethers_1.Contract(
        diamondAddress,
        ...
        for (const cut of facetCuts) {
          console.log(
            chalk_1.default.bold(
              `- ${types_1.FacetCutAction[cut.action]} for facet ${cut.name} at ${cut.facetAddress}`,
            ),
          );
          console.log(
            chalk_1.default.gray(`  Selectors:`),
...
>>>>>>> ddf4b57 (feat: add devcontainer)
      ...
      console.log(
        chalk_1.default.blueBright(
          `🔄 Waiting for DiamondCut transaction to be mined...`,
        ),
      );
      console.log(chalk_1.default.gray(`   Transaction hash: ${tx.hash}`));
      // Wait for transaction confirmation
      ...
      if (!receipt) {
        throw new TransactionFailedError(
          "DiamondCut transaction failed",
          tx.hash,
        );
      }
      if (this.verbose) {
        console.log(
          chalk_1.default.gray(`   Gas used: ${receipt.gasUsed.toString()}`),
        );
        console.log(
          chalk_1.default.gray(`   Block number: ${receipt.blockNumber}`),
        );
      }
      // Update the deployed diamond data
      const txHash = tx.hash;
      ...
      console.error(
        chalk_1.default.red(
          `❌ Failed to perform diamond cut via RPC: ${error.message}`,
        ),
      );
      throw error;
    }
  ...
  async executeInitializerFunctions(diamond) {
    ...
    for (const [
      facetName,
      initFunction,
    ] of diamond.initializerRegistry.entries()) {
      if (this.verbose) {
        console.log(
          chalk_1.default.blueBright(
            `▶ Running ${initFunction} from the ${facetName} facet via RPC`,
          ),
        );
      }
      try {
        ...
        const facetArtifactPath = (0, path_1.join)(
          diamond.contractsPath,
          `${facetContractName}.sol`,
          `${facetContractName}.json`,
        );
        if (!fs.existsSync(facetArtifactPath)) {
          throw new Error(`Facet artifact not found: ${facetArtifactPath}`);
        }
        const facetArtifact = JSON.parse(
          fs.readFileSync(facetArtifactPath, "utf8"),
        );
        const initContract = new ethers_1.Contract(
          diamondAddress,
          facetArtifact.abi,
          this.signer,
        );
        ...
        console.log(
          chalk_1.default.blueBright(
            `🔄 Waiting for ${facetName}.${initFunction} to be mined...`,
          ),
        );
        const receipt = await tx.wait();
        if (!receipt) {
          throw new TransactionFailedError(
            `${facetName}.${initFunction} transaction failed`,
            tx.hash,
          );
        }
        if (this.verbose) {
        ...
        console.log(
          chalk_1.default.green(`✅ ${facetName}.${initFunction} executed`),
        );
      } catch (error) {
        console.error(
          chalk_1.default.red(
            `❌ Failed to execute ${facetName}.${initFunction}: ${error.message}`,
          ),
        );
        throw error;
      }
    ...
    try {
      await this.withRetry(async () => {
        ...
        if (this.verbose) {
          ...
          console.log(
            chalk_1.default.blue(
              `💰 Deployer balance: ${ethers_1.ethers.formatEther(balance)} ETH`,
            ),
          );
        }
        ...
        if (balance < (0, ethers_1.parseUnits)("0.01", 18)) {
          console.warn(
            chalk_1.default.yellow(
              `⚠️ Low balance detected: ${ethers_1.ethers.formatEther(balance)} ETH`,
            ),
          );
        }
      ...
      throw new RPCConnectionError(
        `Failed to validate RPC connection: ${error.message}`,
        error,
      );
>>>>>>> ddf4b57 (feat: add devcontainer)
    }
